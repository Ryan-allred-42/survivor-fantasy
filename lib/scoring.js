...
export async function resolveEpisode(episodeId, eliminatedPlayerId, allEliminatedIds = []) {
  ...
  for (const league of leagues) {
    ...
    for (const pick of leaguePicks) {
      ...
      // New scoring: points on surviving players × eliminated player's multiplier.
      // Points on the eliminated player (or any other player out this episode) don't score.
      const addedScore = computeEpisodeScore(allocations, elimIds, multiplier);
